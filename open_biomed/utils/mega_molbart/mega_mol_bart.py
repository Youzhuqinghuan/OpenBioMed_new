...
from .workflow import BaseGenerativeWorkflow, add_jitter
...
class MegaMolBART(BaseGenerativeWorkflow):
    ...
    def smileslist2embedding(self, smiles_list):
        ...
        pad_mask = pad_mask[:self.max_model_position_embeddings]
        encode_input = {"encoder_input": token_ids, "encoder_pad_mask": pad_mask}
        # print(f"Molecule input shape: {[key + ': ' + str(encode_input[key].shape) for key in encode_input]}")
...
if __name__ == "__main__":
    MegaMolBART_wrapper = MegaMolBART(vocab_path='./checkpoints/moleculestm/megamolbart/bart_vocab.txt', input_dir='./checkpoints/moleculestm/megamolbart/checkpoints', output_dir=None)
    smiles = 'CCO'
    MegaMolBART_wrapper.smileslist2embedding([smiles])